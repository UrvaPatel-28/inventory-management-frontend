          ...
          <tr className="divide-x-4 divide-outline">
            {keys.map((key) =>
              key.endsWith("id") ? null : (
                <th className="px-6 py-4 text-center text-on-primary-container bg-primary-container border-outline border-b-4 uppercase">{key}</th>
              )
            )}
          </tr>
        </thead>
      ...
      <table className="w-full table-auto h-full">
        <tbody className="divide-y divide-outline">
          {Object.entries(data).map(([key, value]) => (
            key.endsWith('id') ? null : <tr className="divide-x divide-outline">
              <td className="text-center text-on-primary-container bg-primary-container py-1 px-3">{key}</td>
              <td>
                <span
                  className={twMerge(
                    "py-1 px-3",
